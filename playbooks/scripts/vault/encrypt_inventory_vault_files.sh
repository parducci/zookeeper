...
###############################################################
# Description  | Ansible vault encrypt all group_vars/*/vault #
...
function display_help {
  echo "encrypt_inventory_vault_files.sh [-i|--inventory-dir-name|-d|--project-dir-path|-h|--help]"
  echo ""
  ...
  echo "                              e.g. 'staging'. Specify '*' to encrypt 'vault' files"
  ...
  echo ""
  echo "   Note: This script uses the 'get_vault_password.py' script to retreive the vault encryption"
  echo "         password from the 'VAULT_PASSWORD' environment variable."
  echo ""
  echo "   Example:"
  echo "     export VAULT_PASSWORD='somePass@123'"
  echo "     encrypt_inventory_vault_files.sh \ "
...
for VFILE in "${VAULT_FILES[@]}"; do
  echo "Ansible vault encrypting file '$VFILE'"

  # Vault encrypt the current "vault" file
  TEMP_FILE=$(mktemp)
  # Encrypt file and messages. Note: All output is sent to stderr.
  ansible-vault encrypt "$VFILE" --vault-password-file "$SCRIPT_DIR/get_vault_password.py" 2> "$TEMP_FILE"
  RETURN_CODE=$?
  RETURN_TEXT_STDERR=$(cat "$TEMP_FILE")
  rm "$TEMP_FILE"

  # Successfully encrypted
  if [[ "$RETURN_TEXT_STDERR" == "Encryption successful" ]]
  then
  ...
  # Already encrypted
  if [[ "$RETURN_TEXT_STDERR" =~ "input is already encrypted" ]]
  then
    echo "Input is already encrypted"
    continue
  fi
